(function() {
  var destination, docco_styles, docco_template, docs_path, ensure_directory, exec, ext, file_exists, fs, generate_documentation, generate_html, generate_index, generate_readme, get_language, highlight, highlight_end, highlight_start, l, languages, parse, parse_args, path, relative_base, showdown, spawn, template, _ref;
  docs_path = "docs";
  ...
  generate_index = function(source, context) {
    var dest, html, index_template, target_dir, title, write_func;
  ...
  parse_args(function(sources, project_name) {
    ...
    return ensure_directory(docs_path, function() {
      var files, indexDir, next_file;
      ...
      if (context.sources.length > 1) {
        return generate_index(indexDir, context);
      }
    });
  });
}).call(this);
